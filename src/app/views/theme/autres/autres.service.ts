...
export class AutresService {
    ...
    delSpecialisations(item: SpecialisationModel): Observable<any> {
        return this.httpClient.delete(`${ENDPOINT_SPECIALISATION}/${item.specialisationId}`);
    }
    ...
    delStructureSanitaire(item: StructureSanitaireModel): Observable<any> {
        return this.httpClient.delete(`${ENDPOINT_STRUCTURE_SANITAIRE}/${item.structuresanitaireId}`);
    }
    ...
    patchRole(role: RoleModel) {
        Object.keys(role).forEach(k => role[k] == null && delete role[k]);
        return this.httpClient.patch(`${ENDPOINT_ROLES}/${role.roleId}`, role);
    }

    delRole(role: RoleModel) {
        return this.httpClient.delete(`${ENDPOINT_ROLES}/${role.roleId}`);
    }
}
